...
// const openImage = (a) => {
//     console.log(a);
//     document.getElementById("image-popup").src = a.src || a;
...
const request = async (prompt) => {
    const payload = {
        prompt: {
            text: `Extract essential parameters from ${prompt} for precise, high-quality, full-size fashion images using Vertex AI Vision on Google Cloud Platform. Prioritize details, avoid unrelated text, and use only text directly contributing to image generation. Instruct the model to prioritize quick and effective dress generation while maintaining quality.Get all this data in 2 lines & produce only sentence & no other punctutaions.`
        }
    };
    // Define the URL for the Google Cloud Language API
    const url = "https://generativelanguage.googleapis.com/v1beta2/models/text-bison-001:generateText?key=AIzaSyC51hnGqn_3ge9VVLNGoZO0i1blez-7ONA";
